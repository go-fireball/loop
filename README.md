...
3. `ai/next_agent.yaml` is minimal baton metadata:
   - required: `next_role`
   - optional: `handoff_notes`, `return_to`, `escalated_by`, `escalation_reason`
...
- `./scripts/check-baton.sh` — validate files, role validity, and minimal YAML structure.
- `./scripts/generate-next-agent.sh <ROLE> [--notes ...] [--return-to ...] [--escalated-by ...] [--escalation-reason ...]` — write minimal baton metadata.
